    ...
    var wc_meta_boxes_product_variations_ajax = {
        ...
        init: function() {
            ...
            $( 'input:submit', postForm ).bind( 'click keypress', function() {
                ...
                //检查价格
                console.log($('#woocommerce-product-data #product-type').val());
                if($('#woocommerce-product-data #product-type').val() == 'simple'){
                    if($('#woocommerce-product-data #_regular_price').val()=='')
                    {
                        alert('产品尚未设置价格，设置路径 : 产品数据->常规->常规售价');
                        $('html,body').animate({ scrollTop: $('#woocommerce-product-data').offset().top-40 }, 1000);
                        return false
                    }
                }
        ...
        save_changes: function( callback ) {
            ...
            $(".woocommerce_variation").each(function(){
                if($(this).find('.wc_input_price').val()=='')
                {
                    msg+= msg==''?'#'+$(this).attr('data-vid'): ',#'+$(this).attr('data-vid');
                    ispass = false;
                }
            });
