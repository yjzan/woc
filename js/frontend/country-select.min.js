...
jQuery( function( $ ) {
    ...
    function init_city_area()
    {

        //加载项
        if($(".yjz_country_select_element").length==0)
        {
            return;
        }

        var state = $('#billing_state').find("option:selected").val();
        var city =  $('#billing_city').find("option:selected").val();
        var area = $('#billing_area').find("option:selected").val();
        ...
        {
            if(city_json_data[i].name == state_array[state])
            {
        ...
        if(state!='')
        {
            for(var i=0;i<city_data.length;i++)
            {
                ...
                if(temp.name !== city)
                {
                    $("#billing_city").append("<option value='"+temp.name+"' data='"+area_json+"'>"+temp.name+"</option>");
                }
                else
                {
                    area_data = temp.area;
                    $("#billing_city").append("<option value='"+temp.name+"' data='"+area_json+"' selected='selected' >"+temp.name+"</option>");
                }

            }
            $('#billing_city').select2({minimumResultsForSearch: -1});
        }

        area_data = area_data==null ? city_data[0].area: area_data;
        ...
        for(var i=0;i< area_data.length;i++)
        {
            ...
            if(temp !== area)
                $("#billing_area").append("<option value='"+temp+"'>"+temp+"</option>");
            else
                $("#billing_area").append("<option value='"+temp+"'  selected='selected' >"+temp+"</option>");

        }
        $('#billing_area').select2({minimumResultsForSearch: -1});
        $('#billing_state').select2({minimumResultsForSearch: -1});

    }

    init_city_area();

    $("#billing_state").on("select2:select",function () {
        ...
        for(var i=0;i<city_json_data.length;i++)
        {
            if(city_json_data[i].name == state_text)
            {
                city_data = city_json_data[i].city;
                break;
            }
        ...
        $("#billing_area").empty();
        var  area_data = city_data[0].area;
        for(var i=0;i< area_data.length;i++)
        {
            var temp = area_data[i];
            $("#billing_area").append("<option value='"+temp+"' >"+temp+"</option>");
    ...
    $("#billing_city").on("select2:select",function (){
        if($(".yjz_country_select_element").length==0)
        {
            return;
        }
        var area_data = jQuery("#billing_city").find("option:selected").attr('data')
